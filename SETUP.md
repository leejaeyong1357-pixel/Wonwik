...
OPIc 영어 말하기 학습 웹앱. 이 문서 순서대로 진행하면 처음부터 운영까지 가능합니다.
...
| 3 | 임직원 명부 | `data/employees.json` | **시연용 가상 10명 + 시연 계정 1명** (운영 전 교체) |
...
| 모델 | 특징 |
|---|---|
| `claude-sonnet-5` | **기본값.** 채점 품질과 속도의 균형 |
| `claude-opus-5` | 가장 정확하지만 응답이 느림 |
| `claude-haiku-4-5` | 가장 빠르고 저렴 |

> 학습자는 마이페이지에서 모델을 직접 고를 수 있습니다. 직접 고른 값은
> `aiModelPinned` 로 표시되어, 나중에 `DEFAULT_MODEL` 을 바꿔도 덮어쓰지 않습니다.
> 반대로 직접 고른 적이 없으면 항상 현재 기본값을 따릅니다.
...
이름을 입력하지 않고 사번·비밀번호만으로 바로 들어갑니다.
비밀번호 등록 절차도 건너뜁니다.

> ⚠️ 운영 시작 전 반드시 제거하세요. 두 곳을 함께 지워야 합니다.
> 1. `app/login/page.tsx` 의 `DEMO_ID` / `DEMO_PW` 상수와 그 분기
> 2. `data/employees.json` 의 사번 `wonik` 항목
...
- **학습**: 유형 1~5(자기소개·설문·돌발·롤플레이·고난도), 질문 듣기(TTS) → 음성 답변(STT) → AI 채점 → 상세 리포트
- **모의고사**: 실제 OPIc 과 동일한 15문항 연속 응시 → 종합 결과지
